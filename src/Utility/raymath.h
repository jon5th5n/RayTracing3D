...
// Create new vector with x and y component
RMDEF Vector2 NewVector2(float x, float y)
{
	Vector2 result = { x, y };
	return result;
}
...
// Floor vector
RMDEF Vector2 Vector2Floor(Vector2 v)
{
	Vector2 result = { floorf(v.x), floorf(v.y) };
	return result;
}

// Ceil vector
RMDEF Vector2 Vector2Ceil(Vector2 v)
{
	Vector2 result = { ceilf(v.x), ceilf(v.y) };
	return result;
}
...
RMDEF float Vector2Mag(Vector2 v)
{
	float result = sqrtf((v.x * v.x) + (v.y * v.y));
	return result;
}

// Calculate vector square magnitude
RMDEF float Vector2MagSqr(Vector2 v)
{
...
RMDEF Vector2 Vector2SetMag(Vector2 v, float newMag)
{
	float tmp = Vector2Mag(v);
	if (tmp == 0)
		return Vector2Zero();
	Vector2 result = Vector2Scale(v, newMag / Vector2Mag(v));
	return result;
}
...
RMDEF Vector2 Vector2DivideByValue(Vector2 v1, float value)
{
	Vector2 result = { v1.x / value, v1.y / value };
	return result;
}
...
RMDEF Vector2 Vector2Normalize(Vector2 v)
{
	if (Vector2Mag(v) == 0)
		return Vector2Zero();
	Vector2 result = Vector2Scale(v, 1 / Vector2Mag(v));
	return result;
}
...
// Get vectors direction in radients
RMDEF float Vector2Direction(Vector2 v)
{
...
// Sets vectors direction in radients
RMDEF Vector2 Vector2SetDirection(Vector2 v, float direction)
{
	Vector2 result;
	float vMag = Vector2Mag(v);
	result.x = vMag * cosf(direction);
	result.y = vMag * sinf(direction);
	return result;
}

// Create new vector with angle and magnitude
RMDEF Vector2 NewVector2Directional(float direction, float magnitude)
{
	Vector2 result = Vector2SetDirection(Vector2One(), direction);
	result = Vector2SetMag(result, magnitude);
	return result;
}

// Rotates vector direction in radients
RMDEF Vector2 Vector2Rotate(Vector2 v, float rads)
{
	float finalDirection = Vector2Direction(v) + rads;
	Vector2 result = Vector2SetDirection(v, finalDirection);
	return result;
}

// Unit Vector with a direction
RMDEF Vector2 Vector2Directional(float direction)
{
	Vector2 result = Vector2SetDirection(Vector2One(), direction);
...
RMDEF bool Vector2AreEqual(Vector2 v1, Vector2 v2)
{
...
// Create new vector
RMDEF Vector3 NewVector3(float x, float y, float z)
{
...
// Square all values of a vector
RMDEF Vector3 Vector3Square(Vector3 v)
{
	Vector3 result = { v.x * v.x, v.y * v.y, v.z * v.z };
	return result;
}
...
RMDEF Vector3 Vector3RotateRoll(Vector3 v, float roll)
{
	Vector2 ryz = Vector2Rotate(NewVector2(v.y, v.z), roll);
	Vector3 result = NewVector3(v.x, ryz.x, ryz.y);

	return result;
}
// Rotate vector around the y axis
RMDEF Vector3 Vector3RotatePitch(Vector3 v, float pitch)
{
	Vector2 rxz = Vector2Rotate(NewVector2(v.x, v.z), pitch);
	Vector3 result = NewVector3(rxz.x, v.y, rxz.y);

	return result;
}

// Rotate vector around the z axis
RMDEF Vector3 Vector3RotateYaw(Vector3 v, float yaw)
{
	Vector2 rxy = Vector2Rotate(NewVector2(v.x, v.y), yaw);
	Vector3 result = NewVector3(rxy.x, rxy.y, v.z);

	return result;
}

// Rotate vector around the three axes by euler angles
RMDEF Vector3 Vector3RotateEuler(Vector3 v, float roll, float pitch, float yaw)
{
	Vector2 ryz = Vector2Rotate(NewVector2(v.y, v.z), roll);
	Vector3 v2 = NewVector3(v.x, ryz.x, ryz.y);

	Vector2 rxz = Vector2Rotate(NewVector2(v2.x, v2.z), pitch);
	Vector3 v3 = NewVector3(rxz.x, v2.y, rxz.y);

	Vector2 rxy = Vector2Rotate(NewVector2(v3.x, v3.y), yaw);
	Vector3 result = NewVector3(rxy.x, rxy.y, v3.z);
...
// Check if vectors are equal
RMDEF bool Vector3AreEqual(Vector3 v1, Vector3 v2)
{
